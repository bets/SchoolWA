...
async function getWAGroups() {
    ...
    var groups = allGroups.reduce((accu, curr, i) => {
        //stop checking if test query is present and group is not test (and oppisite)
        if (queryString.has('test')) {
        ...
        //See more in file: class names regex.txt
        let classMatch = curr.name.match(/(?:[\u05D0-\u05EA]")?[\u05D0-\u05EA]{1,2}'?[\u05F3]?\s?(\d)/);
        ...
        if (classMatch != null && curr.name.includes('תשפ"ה') && !nonClassGroups.some(name => curr.name.includes(name))) {
            let letter = classMatch[0].replace(/[^\u05D0-\u05EA]/g, '');//remove all but letters
            accu.push({
                id: "g" + i,
                type: "class",
                name: letter + classMatch[1],
                letter: letter,
                num: classMatch[1],
                wid: curr.wid
            });
 ...
 */
async function startSend(hasTime = false) {
